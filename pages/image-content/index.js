// pages/image-content/index.js
const { api } = require('../../utils/util.js')

Page({
  data: {
    imageUrl: '',
    emptyIllustration: '',
    loading: true
  },

  onLoad(options) {
    // 获取页面类型参数：recycle-guide(回收分类科普)、withdraw-guide(提现教程)、service-intro(服务介绍)
    const type = options.type || ''
    this.setType(type)
    this.loadImage(type)
    this.initEmptyIllustration()
  },

  /**
   * 设置页面标题
   */
  setType(type) {
    const titles = {
      'recycle-guide': '回收分类科普',
      'withdraw-guide': '提现教程',
      'service-intro': '服务介绍',
      'invite-reward': '邀请奖励'
    }
    const title = titles[type] || '内容'
    wx.setNavigationBarTitle({ title })
  },

  /**
   * 加载图片
   */
  async loadImage(type) {
    try {
      // 配置键名映射
      const configKeys = {
        'recycle-guide': 'recycle_guide_image',
        'withdraw-guide': 'withdraw_guide_image',
        'service-intro': 'service_intro_image',
        'invite-reward': 'invite_reward_image'
      }
      
      const configKey = configKeys[type]
      if (!configKey) {
        this.setData({ loading: false, imageUrl: '' })
        return
      }

      // 从后端获取配置
      const result = await api.getConfigValue(configKey)
      
      if (result && result.success && result.data) {
        const imageUrl = result.data.trim()
        if (imageUrl) {
          this.setData({ imageUrl, loading: false })
          return
        }
      }
      
      // 没有图片，显示空白状态
      this.setData({ imageUrl: '', loading: false })
    } catch (error) {
      console.error('加载图片失败:', error)
      this.setData({ imageUrl: '', loading: false })
    }
  },

  /**
   * 初始化空白状态插图
   */
  initEmptyIllustration() {
    const svg = encodeURIComponent(`
      <svg width="240" height="160" viewBox="0 0 240 160" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stop-color="#09ccb5"/>
            <stop offset="100%" stop-color="#8ea6ff"/>
          </linearGradient>
        </defs>
        <rect x="0" y="0" width="240" height="160" fill="#f7f8fa"/>
        <circle cx="60" cy="80" r="36" fill="url(#g)" opacity="0.25"/>
        <circle cx="160" cy="64" r="28" fill="url(#g)" opacity="0.35"/>
        <rect x="84" y="96" rx="8" ry="8" width="120" height="16" fill="#e7e9f2"/>
        <rect x="84" y="116" rx="8" ry="8" width="72" height="16" fill="#e7e9f2"/>
        <g transform="translate(36,40)">
          <path d="M10 40c0-16 12-28 28-28s28 12 28 28" stroke="#9aa4d6" stroke-width="4" fill="none" stroke-linecap="round"/>
          <circle cx="38" cy="14" r="6" fill="#9aa4d6"/>
        </g>
      </svg>
    `)
    this.setData({ emptyIllustration: `data:image/svg+xml;charset=UTF-8,${svg}` })
  },

  /**
   * 图片加载错误
   */
  onImageError(e) {
    console.error('图片加载失败:', e)
    this.setData({ imageUrl: '' })
  }
})

